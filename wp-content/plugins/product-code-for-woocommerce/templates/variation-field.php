...
woocommerce_wp_text_input(['id' => sprintf('%s_%d',$field_name,$i),'label' => $label,
    ...
    'product-code-for-woocommerce'),'value' => $code,]);

if($displaySecond == 'yes') {
    woocommerce_wp_text_input(['id' => sprintf('%s_%d',$field_name_second,$i),
        'label' => $label_second,'desc_tip' => true,'description' => __($label_second.
        ' refers to a company’s unique internal product identifier, needed for online product fulfillment',
        'product-code-for-woocommerce'),'value' => $code_second,]);
}

?>
    <!-- <label for="product_code_<?php
echo $variation->ID
