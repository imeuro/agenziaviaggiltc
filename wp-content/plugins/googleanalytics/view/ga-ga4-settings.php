<?php
$ga4_exclude_roles = true === is_array($ga4_exclude_roles) ? $ga4_exclude_roles : [];
?>
...
<div class="settings-wrap setup-wrap<?php echo true === $setup_done ? ' hide' : ''; ?>">
    <?php $auth_info = Ga_Admin::getGa4AuthInfo(); ?>
    <div class="ga-step ga-step-1<?php echo false === $has_code && false === $has_property ? ' engage' : ''; ?>">
        ...
        <a id="google-auth-link" href="<?php echo esc_url($auth_info['auth_url']); ?>">
            <img src="<?php echo plugins_url() . '/googleanalytics/assets/images/Google-logo.svg'; ?>" alt="Google Logo">Sign in with Google
        </a>
    </div>
    <div class="ga-step ga-step-2<?php echo false !== $has_code && false === $has_property ? ' engage' : ''; ?>">
        ...
        <label for="ga4-property">
            <select id="ga4-property">
                ...
                <?php foreach($auth_info['properties'] as $account => $properties) :
                    if (false === empty($properties)) : ?>
                    <option disabled><?php echo esc_html($account); ?>:</option>
                    <?php foreach ($properties as $property) : ?>
                        <option value="<?php echo esc_attr($property['name'] ); ?>"><?php echo esc_html($property['displayName'] ); ?></option>
    ...
    <div class="ga-step ga-step-3<?php echo false === $setup_done && false !== $has_code && false !== $has_property ? ' engage' : ''; ?>">
        ...
        <div class="extra-settings">
            ...
            <a id="complete-ga4-setup" class="green-button" href="<?php echo get_admin_url(); ?>admin.php?page=googleanalytics">Complete and go to dashboard</a>
        </div>
    </div>
</div>
                        ...
                        <select id="ga_account_selector" name="googleanalytics-ga4-property">
                            ...
                            <?php foreach($auth_info['properties'] as $account => $properties) :
                                if (false === empty($properties)) : ?>
                                    <option disabled><?php echo esc_html($account); ?>:</option>
                                    <?php foreach ($properties as $property) : ?>
                                        <option value="<?php echo esc_attr($property['name'] ); ?>" <?php echo $property['name'] === $has_property ? 'selected' : ''; ?>>
                                            <?php echo esc_html($property['displayName'] ); ?>
                                        </option>
                                    <?php endforeach; endif; endforeach;?>
                        </select>
                    </td>
