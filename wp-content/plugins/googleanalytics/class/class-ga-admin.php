...
class Ga_Admin {
	 ...
	 * @return void
	 */
	public static function save_ga4_property_selection( $post ) {
     ...
     */
    public static function getGa4Client()
	{
		$client = new Client();
     ...
     * @throws \Google\Exception
     */
	public static function getGa4AuthInfo()
	{
		$client = self::getGa4Client();
		...
		if (isset($token_response['access_token'])) {
			...
			foreach($accounts as $account) {
				if (false === empty($account['id'])) {
					$response       = wp_remote_get('https://analyticsadmin.googleapis.com/v1alpha/properties/?filter=parent%3Aaccounts%2F' . $account['id'] . '&pageSize=1',
						$args);
					...
					$properties[$account['name']] = false === empty($response_array['properties']) ? $response_array['properties'] : '';
				}
			}
		}
